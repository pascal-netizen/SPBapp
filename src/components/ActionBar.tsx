...
export function ActionBar({ onSaveIst, onSaveSoll, onExportPDF, onExportXLSX, onShare }: ActionBarProps) {
  const { t } = useTranslation()
  const [savedIst, setSavedIst] = useState(false)
  const [savedSoll, setSavedSoll] = useState(false)
  const [shared, setShared] = useState(false)

  const handleSaveIst = () => {
    onSaveIst()
    setSavedIst(true)
    setTimeout(() => setSavedIst(false), 1500)
  }

  const handleSaveSoll = () => {
    onSaveSoll()
    setSavedSoll(true)
    setTimeout(() => setSavedSoll(false), 1500)
  }
  ...
  const saveBtnClass = (active: boolean) => `inline-flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border cursor-pointer transition-colors ${
    active
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <button onClick={handleSaveIst} className={saveBtnClass(savedIst)}>
        {savedIst ? checkIcon : saveIcon}
        {savedIst ? t('actions.saved') : t('actions.saveIst')}
      </button>
      <button onClick={handleSaveSoll} className={saveBtnClass(savedSoll)}>
        {savedSoll ? checkIcon : saveIcon}
        {savedSoll ? t('actions.saved') : t('actions.saveSoll')}
      </button>
